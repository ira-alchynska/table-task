...
function createHeaderCell(column) {
  ...
  const buttonArrow = createButton(arrow, btnArrow);

  const buttonMore = createButton(more, btnMore, column.accessor);
  const buttonWrapper = document.createElement("div");
  buttonWrapper.classList.add("button-wrapper");
  buttonWrapper.append(buttonArrow, buttonMore);
...
function createDataRows(columns, countries) {
  // row in body table
...
function createButton(fontAwesomeIcon, buttonClass, accessor) {
...
function toggleMenu() {
  ...
  tr.forEach((th) => {
    const btnMore = th.querySelector(".btn-more");
    btnMore.addEventListener("click", (event) => {
      const accessor = event.currentTarget.dataset.accessor;
      showDropDown(accessor);
    });
  });
}
...
function showDropDown(accessor) {
  const toggleMenu = document.querySelector(".select");
  const column = columns.find((col) => col.accessor === accessor);
  if (column.toggle) {
    column.toggle = false;
